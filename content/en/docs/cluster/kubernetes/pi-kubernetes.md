...
Now login to each of the workers and issue the `kubeadm join` command
...
Since these changes only take place at boot time, you *must reboot*
...
When the node has no pods scheduled to it you can remove it from the
